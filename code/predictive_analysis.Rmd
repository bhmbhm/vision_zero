...
idx <- createDataPartition(intersections$crash_injured_num, p = .6, 
                                  list = FALSE, 
                                  times = 1)
...
y_train <- int_train$crash_injured_num
y_test <- int_test$crash_injured_num
```
...
rf_fit2 <- randomForest(crash_injured_num ~ .,
                        data = int_train,
                        mtry = 54,
                        ntree = 500,
                        importance = TRUE)
...
# Predict on test set
y_rf_hat <- predict(rf_fit2, int_test)
...
y_hat_total <- as.data.frame(predict(rf_fit2, intersections))

colnames(y_hat_total)[1] <- "injury_prediction"
y_hat_total$IntersectionID <- intersection_ids

write.csv(y_hat_total, "rf_overall_predictions.csv")
```
